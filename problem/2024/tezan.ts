/**
 * 背景：前端在遇到多文件上传时会遇到异步任务的并发控制问题，比如上传10个文件，最多允许同时上传3个
...
async function concurrently<D>(options: Option<D>): Promise<void> {
  // 在这里实现
  const { data, mapper, concurrency } = options
  let index = concurrency
  const list = [...data]

  //   while (index < data.length) {}
  for (let index = 0; index < concurrency; index++) {
    const newItem = list.shift(0)
    innerFun(newItem)
  }

  const innerFun = (item) => {
    item.then(mapper).finally(() => {
      const newItem = list.shift(0)
      innerFun(newItem)
    })
  }
  ...
  //     item.then(mapper).finally(() => {
  //       index += 1
  //       list.push(data.slice(index, index + 1))
  //     })
  //   })
}
